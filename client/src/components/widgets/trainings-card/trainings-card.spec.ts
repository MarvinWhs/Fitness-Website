...
describe('TrainingsCard', () => {
  ...
  it('should fetch exercises on connectedCallback', async () => {
    const fetchStub = sinon.stub(window, 'fetch').resolves(
      new Response(JSON.stringify([
        { id: '1', createdAt: Date.now(), name: 'Push Up', description: 'Do push ups', duration: 10, difficulty: 'Easy', image: '' }
      ]))
    );
  ...
  it('should filter exercises based on search term', async () => {
    element.exercises = [
      { id: '1', createdAt: Date.now(), name: 'Push Up', description: 'Do push ups', duration: 10, difficulty: 'Easy', image: '' },
      { id: '2', createdAt: Date.now(), name: 'Pull Up', description: 'Do pull ups', duration: 15, difficulty: 'Medium', image: '' }
    ];
